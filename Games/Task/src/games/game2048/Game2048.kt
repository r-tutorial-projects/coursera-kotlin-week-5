...
fun GameBoard<Int?>.addNewValue(initializer: Game2048Initializer<Int>) {
    val (cell, value) = initializer.nextValue(this) ?: Pair(null, null)
    cell?.let { set(cell, value) }
}
...
fun GameBoard<Int?>.moveValuesInRowOrColumn(rowOrColumn: List<Cell>): Boolean {
    val list = rowOrColumn.map { get(it) }.moveAndMergeEqual { it * 2 }
    rowOrColumn.forEachIndexed { i, cell -> this[cell] = if (i < list.size) list[i] else null }
